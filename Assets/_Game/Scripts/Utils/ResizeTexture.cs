...
{
    [MenuItem("Auto/Resize/ResizeTextureToDiv4 &r", false, -101)]
    ...
    private static Texture2D CaculateTrim(Texture2D texture)
    {
        ...
        for (int i = 0; i < texture.height; i++)
        {
            ...
            {
                if (pixels[i * texture.width + j].a > 0.005f)
                {
                    isBreak = true;
                    break;
                }
        ...
        for (int i = 0; i < texture.width; i++)
        {
            ...
            {
                if (pixels[i + j * texture.width].a > 0.005f)
                {
                    isBreak = true;
                    break;
                }
        ...
        Vector2Int newTexSize = new Vector2Int(texture.width - outline.x, texture.height - outline.y);
        ...
        for (int i = 0; i < colors.Length; i++)
        {
            ...
            colors[i] = pixels[width + (outline.x / 2 + outline.y / 2 * texture.width + (height) * texture.width)];
        }
